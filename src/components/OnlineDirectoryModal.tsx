...
export const OnlineDirectoryModal: React.FC<OnlineDirectoryModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
      <div 
        className="relative w-full max-w-3xl bg-[#140F0A] border border-amber-500/40 rounded-2xl shadow-2xl overflow-hidden p-6 max-h-[88vh] flex flex-col"
      ...
      >
        ...
        <div className="flex items-center justify-between pb-4 border-b border-amber-500/20">
          ...
          <button
            onClick={onClose}
            className="p-1.5 text-amber-200/60 hover:text-white rounded-lg hover:bg-[#221B13] transition"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSearchSubmit} className="my-4 flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-[#1C150E] border border-amber-500/30 rounded-lg pl-9 pr-4 py-2.5 text-xs text-white focus:outline-none focus:border-amber-400 transition"
            />
          </div>
          <button
            type="submit"
            disabled={loading}
            className="px-4 py-2.5 rounded-lg bg-gradient-to-r from-amber-400 to-yellow-500 hover:brightness-110 text-black font-bold uppercase tracking-wider text-xs shadow transition flex items-center gap-1.5 disabled:opacity-50"
          >
        ...
        <div className="overflow-y-auto flex-1 space-y-2.5 pr-1 my-2">
          {loading ? (
            <div className="py-16 text-center space-y-3">
              ...
              <p className="text-xs text-amber-200/70">Discovering active Malayalam radio streams...</p>
            </div>
          ) : stations.length === 0 ? (
            <div className="py-16 text-center text-neutral-400 space-y-2">
              <Radio className="w-10 h-10 mx-auto text-amber-500/40 stroke-1" />
              <p className="text-sm font-semibold text-amber-100">No additional online stations found</p>
                    ...
                    <button
                      ...
                      className={`p-2 rounded-lg transition flex items-center gap-1 text-xs font-bold uppercase tracking-wider ${
                        isPlaying
                          ...
                          : 'bg-[#251D14] hover:bg-[#33271A] text-amber-200 border border-amber-500/30'
                      }`}
                    >
                    ...
                    <button
                      ...
                      className={`p-2 rounded-lg text-xs font-bold uppercase tracking-wider transition flex items-center gap-1 ${
                        isAlreadyAdded
                          ...
                          : 'bg-[#251D14] hover:bg-[#33271A] text-amber-300 border border-amber-500/30'
                      }`}
        ...
        {/* Footer */}
        <div className="pt-3 border-t border-amber-500/20 flex items-center justify-between text-xs text-amber-300/70">
          <span>Found {stations.length} online station streams</span>
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-lg bg-[#221B13] hover:bg-[#2F251A] text-white font-bold uppercase tracking-wider text-xs border border-amber-500/30 transition"
          >
            Close Directory
          </button>
        </div>
