...
function RadioAppContent() {
  ...
  const themeClass = onamTheme === 'azure' 
    ? 'theme-azure bg-sky-50/50' 
    : onamTheme === 'nordic' 
    ? 'theme-nordic bg-slate-100/60' 
    : 'theme-frost bg-[#F8FAFC]';

  return (
    <div className={`min-h-screen ${themeClass} text-slate-900 flex flex-col pb-36 transition-colors duration-300 selection:bg-sky-500 selection:text-white`}>
      {/* Precision Cool Light Atmospheric Background */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div className="absolute -top-40 left-1/2 -translate-x-1/2 w-[1000px] h-[450px] bg-gradient-to-b from-sky-400/10 via-cyan-300/5 to-transparent blur-3xl" />
        <div className="absolute top-1/3 -right-24 w-96 h-96 bg-blue-400/5 blur-3xl rounded-full" />
        <div className="absolute bottom-1/4 -left-24 w-96 h-96 bg-teal-400/5 blur-3xl rounded-full" />
      </div>
      ...
      <main className="max-w-6xl w-full mx-auto px-3 sm:px-6 lg:px-8 pt-3 sm:pt-5 flex-1 flex flex-col relative z-10">
        ...
        {viewMode === 'tuner' ? (
          ...
          <div className="flex-1">
              ...
              <div className="flex items-center justify-between gap-2 pb-2.5 border-b border-slate-200">
                    ...
                    return (
                      <button
                        ...
                        className={`flex-shrink-0 px-3 sm:px-3.5 py-1.5 sm:py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 sm:gap-2 border ${
                          isActive
                            ? 'bg-gradient-to-r from-sky-500 via-cyan-500 to-blue-600 text-white border-sky-400 font-extrabold shadow-md shadow-sky-500/25 scale-[1.02]'
                            : 'bg-white text-slate-700 hover:text-slate-900 border-slate-200 hover:border-sky-300 hover:bg-slate-50 shadow-sm'
                        }`}
                      >
                        <span className={isActive ? 'text-white' : cat.id === 'favorites' ? 'text-rose-500' : 'text-sky-600'}>
                          {cat.icon}
                        </span>
                ...
                {searchQuery && (
                  <button
                    ...
                    className="flex-shrink-0 text-xs text-sky-600 hover:text-sky-800 flex items-center gap-1 font-bold pl-2"
                  >
            ...
            {activeCategory === 'air' && (
              <div className="mb-4 p-3.5 sm:p-4 rounded-xl bg-gradient-to-r from-sky-50 via-cyan-50/50 to-blue-50 border border-sky-200 text-slate-800 text-xs shadow-sm">
                <div className="flex items-start gap-2.5">
                  <AlertCircle className="w-4 h-4 sm:w-5 sm:h-5 text-sky-600 flex-shrink-0 mt-0.5" />
                  <div className="flex-1">
                    <div className="flex items-center justify-between flex-wrap gap-1">
                      <span className="font-bold text-sky-900 uppercase tracking-wider text-[11px] sm:text-xs">
                        Akashvani (AIR) Stream Link Notice
                      </span>
                      <button
                        onClick={() => setIsContactModalOpen(true)}
                        className="text-[10px] text-sky-700 hover:text-sky-900 font-bold underline flex items-center gap-1"
                      >
                    ...
                    <p className="mt-1 text-[11px] text-sky-800/80 font-malayalam">
            ...
            {filteredStations.length === 0 ? (
              <div className="py-16 text-center text-slate-500 space-y-4 max-w-md mx-auto">
                <Radio className="w-12 h-12 text-sky-400/60 mx-auto stroke-1" />
                <div>
                ...
                <div className="flex items-center justify-center gap-3 pt-2">
                  <button
                    ...
                    className="px-4 py-2 rounded-lg bg-white hover:bg-slate-50 border border-slate-300 text-slate-700 hover:text-slate-900 text-xs font-semibold uppercase tracking-wider transition shadow-sm"
                  >
                    View All Stations
                  </button>
                </div>
            ...
            <div className="mt-8 pt-4 pb-2 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-500">
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={() => setIsContactModalOpen(true)}
                  className="hover:text-sky-700 text-sky-600 font-medium flex items-center gap-1 transition"
                >
